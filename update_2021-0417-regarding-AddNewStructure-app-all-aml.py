@app.route('/xml_to_cif2/<string:filename>/', methods=['GET'])
#@app.cache.cached(timeout=0)
def get_cif_from_xml2(filename='test.cif'):

    result={}
    cif_found=""
    if filename is not None:
        try:
            url="http://127.0.0.1/temp/"+filename
            print(url)
...
def inserting_new_to_aml_m():
    if request.method=='POST':
        ...
        idxcol=myclient["mp"]["mp_last_task_id_for_new"]
        idxdoc=idxcol.find_one()
        last_idx=idxdoc['last_idx']
        taskid="aml-m-"+str(last_idx+1)
        print(taskid)
        new_material['task_id']=taskid
        idxcol.update_one({"last_idx":last_idx},{"$set":{"last_idx":(last_idx+1)}})
        ...
        composition=new_material['pretty_formula']
        comp=None
        try:
        ...
        elements.sort()
        new_material['elements']=elements
